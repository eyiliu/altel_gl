...
public:
  std::vector<std::shared_ptr<TelescopeData>> m_vec_ring_ev;
  std::shared_ptr<TelescopeData> m_ring_end; // ring end is nullptr,therefore real data as nullptr should not go into the ring.
  ...
  void pushBufferEvent(std::shared_ptr<TelescopeData> df){ //by write thread
  ...
  std::shared_ptr<TelescopeData>& frontBufferEvent(){ //by read thread
